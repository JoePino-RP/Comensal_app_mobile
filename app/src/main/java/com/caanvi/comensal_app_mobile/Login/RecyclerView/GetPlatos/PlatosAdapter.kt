...
class PlatosAdapter (private val _restaurant:List<Platos>, val onClickListener: OnClickListener):  RecyclerView.Adapter<PlatosAdapter.PlatosViewHolder>() {
    ...
    override fun onBindViewHolder(holder: PlatosViewHolder, position: Int) {
        ...
        binding.txtNombrePlato.text = "Nombre: "+ _restaurant[position].nombre_pla
        binding.txtPrecioPlato.text = "Precio: "+_restaurant[position].precio_pla
        binding.txtDescripcionPlato.text ="Descripcion: "+ _restaurant[position].descripcion_pla
       // binding.txtTelefono.text ="Teléfono: "+ _restaurant[position].
        Picasso.get().load(_restaurant[position].imagen_pla).into(binding.imgRestaurant)
